...
function App() {
  ...
  function confirmResetSample() {
    ...
    setNotice('Sample filters and saved insights were reset. The draft note was cleared.')
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      {page === 'dashboard' && <>
        ...
        {(notice || actionError) && <div className={actionError ? 'toast error' : 'toast'} role={actionError ? 'alert' : 'status'} aria-live="polite">{actionError || notice}<button type="button" onClick={() => { setNotice(''); setActionError('') }} aria-label="Dismiss message">×</button></div>}

        {resetPending && <div className="reset-dialog-backdrop" role="presentation"><section ref={resetDialogRef} className="reset-dialog" role="dialog" aria-modal="true" aria-labelledby="reset-title" aria-describedby="reset-description"><h2 id="reset-title">Reset this sample?</h2><p id="reset-description">This clears all saved insights, returns filters to <b>All · 30 days</b>, and discards the draft note in the editor.</p><p>Your current notes and filter scope can be recovered with <b>Undo notebook change</b> only after the reset.</p><div><button type="button" ref={cancelResetRef} className="secondary" onClick={() => setResetPending(false)}>Keep working</button><button type="button" className="primary" onClick={confirmResetSample}>Reset notes and filters</button></div></section></div>}
